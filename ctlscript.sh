...
# Checking for errors
for e in $APACHE_ERROR $PROFTP_ERROR $MYSQL_ERROR $SUBVERSION_ERROR $TOMCAT_ERROR $RESIN_ERROR $MEMCACHED_ERROR $INGRES_ERROR $OPENOFFICE_ERROR $LUCENE_ERROR $ZOPE_ERROR $POSTGRESQL_ERROR $THIRD_ERROR $NAGIOS_ERROR $RABBITMQ_ERROR $JETTY_ERROR $JBOSS_ERROR $HYPERSONIC_ERROR; do
    if [ $e -gt 0 ]; then
        ERROR=$e
    fi
done
