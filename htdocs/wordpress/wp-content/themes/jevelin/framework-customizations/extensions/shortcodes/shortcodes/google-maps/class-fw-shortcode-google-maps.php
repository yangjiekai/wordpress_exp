...
class FW_Shortcode_Google_Maps extends FW_Shortcode {
		...
		if (empty($this->data)) {
			$this->data = apply_filters('fw_shortcode_map_provider', array(
				'custom' => array(
					...
					'label'      => esc_html__('Custom', 'jevelin'),
					'options'    => array(
						'locations' => array(
							'label' => esc_html__('Locations', 'jevelin'),
							'popup-title' => esc_html__('Add/Edit Location', 'jevelin'),
							'type' => 'addable-popup',
							'desc' => false,
							'template' => '{{  if (location.location !== "") {  print(location.location)} else { print("' . esc_html__('Note: Please set location', 'jevelin') . '")} }}',
							'popup-options' => array(
								'location' => array(
									'type' => 'map',
									'label' => esc_html__('Location', 'jevelin'),
								),
								'title' => array(
									'type' => 'text',
									'label' => esc_html__('Location Title', 'jevelin'),
									'desc' => esc_html__('Set location title', 'jevelin'),
								),
								'description' => array(
									'type'  => 'textarea',
									'label' => esc_html__('Location Description', 'jevelin'),
									'desc'  => esc_html__('Set location description', 'jevelin')
								),
								'url' => array(
									'type'  => 'text',
									'label' => esc_html__('Location Url', 'jevelin'),
									'desc'  => esc_html__('Set page url (Ex: http://example.com)', 'jevelin'),
								),
								'thumb' => array(
									'label'       => esc_html__('Location Image', 'jevelin'),
									'desc'        => esc_html__('Add location image', 'jevelin'),
									'type'        => 'upload',
								)
							)
						)
	...
	public function _callback_get_custom_locations($atts) {
		...
		if (!empty($rows)) {
			foreach($rows as $key => $row) {
				...
				$result[$key]['thumb']       = fw_resize(wp_get_attachment_url(fw_akg('thumb/attachment_id', $row)), 180, 180, true);
	...
	protected function _render($atts, $content = null, $tag = '')
	{
		if (!isset($atts['data_provider']['population_method'])) {
			trigger_error(
				esc_html__('No location provider specified for map shortcode', 'jevelin')
			);
			return '<b>' . esc_html__( 'Map Placeholder', 'jevelin' ) . '</b>';
		}
		...
		if (!isset($this->data[$provider])) {
			return '<!-- WARNING: '
			       . sprintf(esc_html__('Unknown location provider "%s" specified for map shortcode', 'jevelin'), $provider)
			       . ' -->';
		}
